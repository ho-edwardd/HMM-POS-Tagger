...
def infer_sentences(model, sentences, start, beam_width):
    """
    Args:
        ...
        beam_width (int): beam width for beam search. If beam_width=1, greedy decoding is used.
    ...
    for i in range(len(sentences)):
        res[start + i] = model.inference(sentences[i], beam_width=beam_width)
    return res
...
def load_data(sentence_file, tag_file=None):
    ...
    doc_start_indexes = df_sentences.index[
        df_sentences["word"] == "-DOCSTART-"
    ].tolist()
...
def confusion_matrix(tag2idx, idx2tag, pred, gt, fname):
    ...
    df_cm = pd.DataFrame(
        matrix,
        index=[idx2tag[i] for i in range(len(tag2idx))],
